...
def add_signal(df, data_type):
    ...
    if data_type == "daily":
        for i in range(len(df['Predicted'])):
            if df['Predicted'].iat[i] >= df['BU'].iat[i]:           # sell signal     daily
                buy_list.append(np.nan)
                sell_list.append(df['Predicted'].iat[i])
            elif df['Predicted'].iat[i] <= df['BL'].iat[i]:         # buy signal      daily
                buy_list.append(df['Predicted'].iat[i])
...
def plot_signals(df, ticker, data_type):
    '''
    ...
    data_type: string containing type of data (daily, weekly, monthly)
    ...
    plt.title(f'Bollinger Bands {data_type} chart ' + str(ticker))
    ...
    plt.scatter(df['Date'], df['Buy'], label='Buy', marker='^', color = 'red')
    plt.scatter(df['Date'], df['Sell'], label='Sell', marker='v', color = 'orange')

    plt.legend()

    # plt.show()
    plt.savefig(f'../plots/{ticker}_{data_type}.png')